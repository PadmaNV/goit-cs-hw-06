...
def save_to_db(data):
    ...
    db = client.homework
    # username=Padma&message=test  -> {"date": "2022-10-29 20:20:58.020261","username": "Padma", "message": "test"}
    try:
        data = unquote_plus(data)
        parse_data = dict(item.split("=") for item in data.split("&"))
        parse_data["date"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
        ordered_data = OrderedDict(
            (key, parse_data[key]) for key in ["date", "username", "message"]
        )
        print(parse_data)
        db.messages.insert_one(ordered_data)
    except Exception as e:
        logging.error(e)
    finally:
        client.close()
